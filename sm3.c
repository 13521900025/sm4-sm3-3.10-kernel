...
static void sm3_compress(u32 digest[8], const u8 block[64]);
...
static int sm3_update(struct shash_desc *desc, const u8 *data, unsigned int data_len)
{
	struct sm3_ctx *ctx = shash_desc_ctx(desc);
...
static int sm3_final(struct shash_desc *desc, u8 *digest)
{
	struct sm3_ctx *ctx = shash_desc_ctx(desc);
	int i;
